	...
	for($i = 0; $i < $count; $i++){
		...
		if (preg_match_all('#<video>([\\s\\S]*?)</video>#', $broadcastTag, $matches3)) {
			...
			for($k = 0; $k < $count2; $k++){
				...
				$txt = '	<description><![CDATA['.$description.' <a id="music" href="http://cablev2.com/tv/play.php?channelid='.$location_name.'" class="watch_now" target="tl">Watch Channel</a> ]]></description>'."\n";
				fwrite($output, $txt);
