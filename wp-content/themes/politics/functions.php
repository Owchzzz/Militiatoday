...
function themerex_init_template() {
	...
	// Group menu
	global $THEMEREX_groupmenu;
	if (get_custom_option('show_top_panel')!='hide') {
		$menu_slug = get_custom_option('menu_group');
		$args = array(
			'menu'              => empty($menu_slug) || $menu_slug=='default' || is_inherit_option($menu_slug) ? '' : $menu_slug,
			'container'         => '',
			'container_class'   => '',
			...
			'link_after'        => '',
			'depth'             => 11,
			'theme_location'    => 'groupmenu'
		);
		if (get_theme_option('custom_menu')=='yes' && class_exists('themerex_walker')) {
			$args['walker'] = new themerex_walker;
		}
		$THEMEREX_groupmenu = wp_nav_menu($args);
	} else
		$THEMEREX_groupmenu = '';
...
// Add group menu classes
if ( !function_exists( 'themerex_nav_menu_classes' ) ) {
	function themerex_nav_menu_classes($items, $args) {
		if (is_admin()) return $items;
		if ($args->menu_id == 'groupmenu' && get_theme_option('menu_colored')=='yes') {
			foreach($items as $k=>$item) {
				if ($item->menu_item_parent==0) {
					if ($item->type=='taxonomy' && $item->object=='category') {
						$cur_theme = get_category_inherited_property($item->object_id, 'blog_theme');
...
add_filter( 'wp_nav_menu_items', 'group_menu_customize', 10, 2 );


function group_menu_customize ( $items, $args ) {
    if ($args->menu == 'group-menu') {
        ...
        // Find news-2
        $items = str_replace("news-2",  $root.'/'.$root.'-news', $items);


        // Find local-events
        $items = str_replace("local-events", $root.'/'.$root.'-local-events', $items);

        // local-business
        $items = str_replace("local-businesses", $root.'/'.$root.'-local-businesses', $items);

		// Obituaries
		$items = str_replace("obits", $root.'/'.$root.'-obituaries', $items);
		
		// Replace "Butler Channels" with <County> Channels
		// Replace the webtv url with the correct one.
		// ABSOLUTE KLUDGE FOR NOW
		if ($root == "butler-county-pa") {
			$items = str_replace("webtv/", "u/butler-county-pa/play.php?channelid=default", $items);
		}
		else if ($root = "st-louis-county-mo") {
			$items = str_replace("Butler Channels", "St. Louis Channels", $items);
			$items = str_replace("webtv/", "u/st-louis-county-mo/play.php?channelid=default", $items);
		}
